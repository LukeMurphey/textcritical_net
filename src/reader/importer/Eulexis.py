...
class EulexisAnalysesImporter(LineImporter):
    """
    The Eulexis analyses importer imports the analyses file (greek-analyses.txt) from the Eulexis project.

    See:
    * https://raw.githubusercontent.com/PhVerkerk/Eulexis_off_line/master/Eulexis_data/trad_gr_en_fr_de.csv
    * https://github.com/PhVerkerk/Eulexis_off_line/blob/master/Eulexis_data/analyses_gr.txt
    ...
    +------+--------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------+
    |    1 | parse_entry        | Get the form    | 'ca/ch|s	{e)ca_/ch|s,e)ca/gnumi	aor subj act 2nd sg}{e)ca/ch|s,e)ca/gw	aor subj act 2nd sg}                                                           |
    |    2 | parse_descriptions | Parse the forms | {e)ca_/ch|s,e)ca/gnumi	aor subj act 2nd sg}{e)ca/ch|s,e)ca/gw	aor subj act 2nd sg}                                                                       |
    |    3 | parse_description  | Parse the form  | {e)ca_/ch|s,e)ca/gnumi	aor subj act 2nd sg}                                                                                                               |
    +------+--------------------+-----------------+------------------------------------------------------------------------------------------------------------------------------------------------------------+
    ...
    def import_line(cls, entry, line_number=None, raise_exception_on_match_failure=False):
        ...
        for desc in cls.PARSE_ANALYSIS_DESCRIPTIONS_RE.findall(entry):
            ...
            EulexisAnalysesImporter.import_analysis_entry(
...
class EulexisLemmataImporter():
    """
    The Eulexis Lemmata importer imports the lemmata file (trad_gr_en_fr_de.csv) from the Eulexis project.
    ...
    def parse_lemma(entry, line_number=None):
        """
        Parse an entry in the Eulexis greek-lemmata.txt file.
