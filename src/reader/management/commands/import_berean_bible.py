...
class Command(BaseCommand):
    ...
    help = "Imports Berean Bible documents"
    ...
    def handle(self, *args, **options):
        ...
        import_policy_file = os.path.join(os.path.split(sys.argv[0])[0], "reader", "importer", "berean_bible_import_policy.json")
        
        import_policy = JSONImportPolicy()
        import_policy.load_policy(import_policy_file)
        
        print("Importing", os.path.basename(filename))
        importer = BereanBibleImporter(import_policy=import_policy.should_be_processed)
